...
pub fn move_players(
    ...
    mut q: Query<(&Player, &mut Body, &mut Skills, &mut Inputs, &Vitals)>,
) {
    for (player, mut body, mut skills, mut inputs, vitals) in &mut q {
        // Jede empfangene Eingabe wird genau einmal simuliert - nicht eine je
        // Tick. Der Client sagt jede gesendete Eingabe voraus; führte der
        // Server nur einen Teil davon aus, zöge ihn jeder Abgleich sichtbar
        // zurück.
        for frame in inputs.take_for_tick() {
