...
import { VOLLES_TEMPO, gliedmassen, ruhe, schritt } from "../client/js/laufzyklus.js";
...
function ausschlag(zustand) {
  ...
  return Math.max(
    Math.abs(w.beinLinks),
    Math.abs(w.beinRechts),
    Math.abs(w.armLinks),
    Math.abs(w.armRechts),
    Math.abs(w.wippen),
  );
}
...
console.log("Laufen");
{
  ...
  pruefe("schlaegt aus", Math.abs(w.beinLinks) > 0.3, `linkes Bein nur ${w.beinLinks.toFixed(3)}`);
  pruefe(
    "Beine gegengleich",
    w.beinLinks * w.beinRechts < 0,
...
console.log("Anhalten");
{
  ...
  pruefe(
    ...
    ausschlag(nachEinerSekunde) < 0.01,
  ...
  pruefe(
    "und bleiben dort",
    ausschlag(nachDrei) <= ausschlag(nachEinerSekunde) + 1e-9,
...
console.log("Langsam gehen");
{
  ...
  pruefe(
    ...
    halb.ausschlag < voll.ausschlag * 0.7,
...
console.log("Zahlenbereich");
{
  ...
  pruefe(
    "Phase bleibt klein",
    Math.abs(weit.phase) <= Math.PI * 2,
    `Phase ${weit.phase} nach zehn Minuten Laufen`,
  );
  pruefe(
    "und ist eine Zahl",
    Number.isFinite(weit.phase) && Number.isFinite(weit.ausschlag),
